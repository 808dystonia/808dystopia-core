// Step 4: download just the selected highlight's time range, crop/resize
// to 9:16, and overlay the 808dystopia watermark (top-left corner) via
// ffmpeg. Uses the same placeholder logo.png the carousel pipeline already
// accepts as a stand-in for the official brand asset -- swap both at once
// whenever the real files are provided.
//
...
import { cropAndWatermark } from "../clients/ffmpeg.js";
...
export async function processClip(video) {
  ...
  await cropAndWatermark(rawClipPath, WATERMARK_PATH, finalClipPath);
