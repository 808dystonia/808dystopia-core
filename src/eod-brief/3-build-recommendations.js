...
function buildPrompt(accomplishments, analytics) {
  const accomplishmentsText = accomplishments.length
    ...
    : "(none today)";
  const analyticsEntries = Object.entries(analytics.instagram);
  const analyticsText = analyticsEntries.length
    ? analyticsEntries.map(([key, value]) => `${key}: ${value}`).join(", ")
    : "(no data available)";

  return `You are a sharp, concise ops advisor for a small underground hip-hop media business (808 Dystopia) run by two people, STOKELY and FRZA. Read today's actual dev accomplishments and Instagram analytics below and return ONLY a JSON object (no markdown, no other text) with this field:
